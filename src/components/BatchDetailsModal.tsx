...
import { X, Calendar, Clock, Users, BookOpen, User, MapPin, CheckCircle, XCircle, Sparkles, Loader2 } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <div
          className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto pointer-events-auto animate-scaleIn"
          ...
          <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-6 rounded-t-2xl">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <h2 className="text-2xl font-bold mb-2">{batch.batch_name}</h2>
            ...
            <div className="mt-4 flex gap-2">
              {isFull && (
                <span className="inline-flex items-center gap-1 px-3 py-1 bg-red-500 text-white rounded-full text-xs font-bold">
                  <XCircle className="w-4 h-4" />
                  BATCH FULL
                </span>
              )}
              {isAlreadyEnrolled && (
                <span className="inline-flex items-center gap-1 px-3 py-1 bg-gray-500 text-white rounded-full text-xs font-bold">
                  <CheckCircle className="w-4 h-4" />
                  ALREADY ENROLLED
                </span>
              )}
              {!isFull && !isAlreadyEnrolled && (
                <span className="inline-flex items-center gap-1 px-3 py-1 bg-white text-blue-600 rounded-full text-xs font-bold">
              ...
              {batch.teachers?.users?.name && (
                <div className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{batch.teachers.users.name}</p>
                  </div>
                </div>
              )}
