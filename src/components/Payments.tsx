...
const Payments: React.FC<PaymentsProps> = ({ isEmbedded = false }) => {
  ...
  useEffect(() => {
    ...
    const checkAndSetPaymentLock = async () => {
      try {
        ...
        if (json.success && json.data?.payment_type) {
          ...
          setIsLocked(true);
          // console.log("🔒 Payment lock found for batch", selectedEnrollmentId + ":", json.data.payment_type);
        } else {
          setIsLocked(false);
          // console.log("🔓 No payment lock for batch", selectedEnrollmentId);
        }
      } catch (error) {
        // console.log("⚠️ Payment lock check failed:", error);
        setIsLocked(false);
      }
    };
    ...
    if (transactions.length === 0) {
      // console.log("⏳ No transactions loaded yet, checking lock status only for:", selectedEnrollmentId);
      checkAndSetPaymentLock();
      return;
    }

    // console.log("🔄 Batch switched - Resetting payment states for:", selectedEnrollmentId);

    // Reset all payment states first (clean slate for new batch)
    ...
    console.log("📊 Transactions for selected batch:", relevantTxns);
    ...
    console.log("💰 Full payment status for this batch:", fullPaid ? "PAID ✅" : "NOT PAID ❌");
    ...
    console.log("📅 Paid EMI months for this batch:", paidEmiMonths.length > 0 ? paidEmiMonths : "None");
    setPaidMonths(paidEmiMonths);

    // Check payment lock status for THIS batch
    checkAndSetPaymentLock();
  }, [selectedEnrollmentId, transactions, registrationNumber, enrollments.length]);
  ...
  if (isEmbedded) {
    return (
      <div className="w-full">
        ...
        <div className="relative bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 mb-6 sm:mb-8 shadow-2xl overflow-hidden">
          {/* Background Pattern */}
        ...
        {hasMultipleEnrollments ? (
          <div className="mb-4 sm:mb-6">
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg sm:rounded-xl p-4 sm:p-6 border border-blue-200 shadow-md">
          ...
          <div className="mb-4 sm:mb-6">
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-lg sm:rounded-xl p-4 sm:p-5 shadow-lg">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <div className="hidden sm:block">
            <div className="flex gap-2 sm:gap-4 bg-gradient-to-r from-blue-50 to-indigo-50 p-2 rounded-xl border border-blue-100">
              <button
          ...
          <div className="w-full mt-4 sm:mt-6">
            {activeTab === "current" && (
              <div className="grid grid-cols-1 gap-4 sm:gap-6">
                ...
                <div className="bg-blue-50 rounded-xl sm:rounded-2xl shadow-xl overflow-hidden border border-blue-200">
                  {/* Gradient Blue Header */}
                  <div className="px-4 sm:px-6 py-4 sm:py-5 bg-gradient-to-r from-blue-600 to-blue-400">
                    <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div className="w-full p-4 sm:p-6">
                    {result ? (
                      <div className="bg-white p-4 sm:p-6 rounded-lg sm:rounded-xl shadow-md border border-blue-100 mb-4 sm:mb-6">
                        {/* Registration & Course */}
                        <div className="mb-3 sm:mb-4">
                          <label className="block text-xs sm:text-sm font-semibold text-blue-700 mb-1.5 sm:mb-2">
                            Registration Number
                          </label>
                          <input
                            type="text"
                            ...
                            className="block w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 border-blue-300 rounded-lg shadow-sm bg-blue-50 text-gray-800 text-sm sm:text-base font-medium focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400"
                          />
                        </div>
                        ...
                        {/* Divider */}
                        <div className="my-4 sm:my-6 border-t-2 border-blue-200"></div>

                        {/* Payment Type Selection */}
                        <div className="mb-4 sm:mb-6">
                          <label className="block text-xs sm:text-sm font-semibold text-blue-700 mb-2 sm:mb-3">
                            Select Payment Type
                          </label>
                          <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:gap-4">
                            <label className="flex items-center bg-white px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg border-2 border-blue-300 cursor-pointer hover:bg-blue-50 transition-all duration-200 flex-1 sm:flex-initial">
                              <input
                                type="radio"
                                name="paymentType"
                                value="full"
                                ...
                                className="w-4 h-4 text-blue-600 focus:ring-blue-400 flex-shrink-0"
                              />
                              <span className="ml-2 sm:ml-3 font-semibold text-sm sm:text-base text-gray-700">Full Fees</span>
                            </label>
                            ...
                            {result.duration > 1 && (
                              <label className="flex items-center bg-white px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg border-2 border-blue-300 cursor-pointer hover:bg-blue-50 transition-all duration-200 flex-1 sm:flex-initial">
                                <input
                                  ...
                                  className="w-4 h-4 text-blue-600 focus:ring-blue-400 flex-shrink-0"
                                />
                                <span className="ml-2 sm:ml-3 font-semibold text-sm sm:text-base text-gray-700">Monthly Fees</span>
                              </label>
                            )}
                            ...
                            {!isLocked && (
                              <button
                                ...
                                className="w-full sm:w-auto sm:ml-auto px-4 sm:px-6 py-2.5 sm:py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold text-sm sm:text-base shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2"
                              >
                        ...
                        {paymentType === "full" && (
                          <div className="bg-gradient-to-br from-blue-50 to-white p-4 sm:p-6 rounded-lg sm:rounded-xl shadow-md border border-blue-200 mb-3 sm:mb-4">
                            ...
                            <div className="space-y-3 sm:space-y-4">
                              <div>
                                ...
                                <input
                                  ...
                                  readOnly
                                  value={`₹${totalFees}`}
                                  className="block w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 border-blue-300 rounded-lg shadow-sm bg-white text-gray-800 text-sm sm:text-base font-semibold focus:outline-none focus:ring-2 focus:ring-blue-400"
                                />
                              </div>
                              ...
                              {/* Divider */}
                              <div className="border-t-2 border-blue-200 my-3 sm:my-4"></div>

                              <div>
                                <label className="block text-xs sm:text-sm font-semibold text-blue-700 mb-1.5 sm:mb-2">
                                  Final Amount to Pay
                                </label>
                                <div className="bg-gradient-to-r from-blue-600 to-blue-500 p-3 sm:p-4 rounded-lg shadow-lg">
                              ...
                              {/* Pay Button */}
                              <button
                                className={`w-full px-4 sm:px-6 py-3 sm:py-4 text-base sm:text-lg font-bold rounded-lg shadow-lg transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 ${
                                  statusApproved
                                ...
                                disabled={statusApproved || isPaying || isVerifying}
                              >
                                {statusApproved ? (
                                  <>
                            ...
                            <div className="mt-3 sm:mt-4 bg-blue-100 border-2 border-blue-300 rounded-lg p-3 sm:p-4">
                              <div className="flex gap-2 sm:gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  <h5 className="font-bold text-blue-800 text-sm sm:text-base mb-1.5 sm:mb-2">
                                  ...
                                  <ul className="text-xs sm:text-sm text-blue-700 space-y-1 sm:space-y-1.5">
                                    ...
                                    <li className="flex items-start gap-1.5 sm:gap-2">
                                      <span className="text-blue-600 font-bold flex-shrink-0">•</span>
                                      <span>Please <strong>stay on this page</strong> and wait for the green success message that says "Payment Verified".</span>
                                    </li>
                                    <li className="flex items-start gap-1.5 sm:gap-2">
                                      <span className="text-blue-600 font-bold flex-shrink-0">•</span>
                                      <span>This usually takes just a few seconds. You'll see a loading animation while we confirm your payment.</span>
                                    </li>
                                    <li className="flex items-start gap-1.5 sm:gap-2">
                                      <span className="text-blue-600 font-bold flex-shrink-0">•</span>
                                      <span>Don't worry if it takes a moment - we're making sure your payment is recorded correctly. Your money is safe!</span>
                                    </li>
                                  </ul>
                                </div>
                              </div>
                              ...
                              return (
                                <>
                                  ...
                                  <div className="mb-4 sm:mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                                    ...
                                    <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-indigo-200 shadow-sm">
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <button
                                      ...
                                      disabled={isDisabled || isPaying || isVerifying}
                                    >
                                      {isPaid ? (
                                        <>
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
            <div className="flex justify-between h-14 sm:h-16">
              <div className="flex items-center">
                <h1 className="text-base sm:text-lg md:text-xl font-semibold text-gray-900 tracking-wide">
                  Payment Management
                </h1>
              </div>
            </div>
                    ...
                    <div className="bg-white p-6 rounded-xl shadow-md border border-blue-100">
                      ...
                      {isLocked && paymentType === "full" && (
                        <div className="bg-gradient-to-br from-blue-50 to-white p-6 rounded-xl shadow-md border border-blue-200">
                          <button
                            onClick={() => handlePayment(finalFees, 0)}
                            disabled={isPaying || statusApproved}
